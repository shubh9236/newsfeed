...
function codex_book_init() {
	$labels = array(
		...
		'menu_name'          => _x( 'NewsFeed', 'admin menu', 'your-plugin-textdomain' ),
...
function myplugin_register_form() {
    ...
    function myplugin_registration_errors( $errors, $sanitized_user_login, $user_email ) {
		...
		$errors->add( 'passwords_not_matched', __('<strong>ERROR</strong>: Passwords must match' ,'mydomain') );
	}
		if ( strlen( $_POST['password'] ) < 4 ) {
		$errors->add( 'password_too_short', __('<strong>ERROR</strong>: Password must be at least four characters long','mydomain') );
	}
        return $errors;
    }
...
add_action( 'after_setup_theme', 'disable_dashboard' );
 
/**
 * Redirect back to homepage and not allow access to
 * WP admin for Subscribers.
 */
function custom_redirect_admin(){
    if ( ! defined('DOING_AJAX') && ! current_user_can('edit_posts') ) {
        wp_redirect( site_url() );
        exit;      
    }
}
add_action( 'admin_init', 'custom_redirect_admin' );?>
